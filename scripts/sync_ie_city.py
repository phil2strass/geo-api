...
IE_COUNTRY_QID = "Q27"
IE_SETTLEMENT_CLASS_QIDS = ("Q515", "Q3957", "Q1549591")
IE_CITY_SOURCE = "wikidata.city.ie.city_or_town"


@dataclass(frozen=True)
class IeSettlementRow:
    wikidata_id: str
    name: str | None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Populate city with real Ireland urban settlements (Wikidata city/town/big city)."
        )
    )
    parser.add_argument(
        "--host",
...
def build_settlement_query() -> str:
    classes = " ".join(f"wd:{qid}" for qid in IE_SETTLEMENT_CLASS_QIDS)
    return f"""
SELECT DISTINCT ?item ?itemLabel ?itemDescription WHERE {{
  VALUES ?settlementClass {{ {classes} }}
  ?item wdt:P31 ?settlementClass .
  ?item wdt:P17 wd:{IE_COUNTRY_QID} .
...
def load_country_id(args: argparse.Namespace) -> int:
    ...
    if len(rows) != 1:
        raise RuntimeError(f"Expected one IE country row, got {len(rows)}")
...
def build_city_rows(
    *,
    country_id: int,
    settlements: list[IeSettlementRow],
...
def apply_sync(args: argparse.Namespace, *, country_id: int, city_rows: list[dict[str, Any]]) -> None:
    with tempfile.TemporaryDirectory(prefix="ie-city-sync-") as tmp_dir:
...
DELETE FROM city c
WHERE c.country_id = {country_id}
  AND c.source = {sql_quote(IE_CITY_SOURCE)}
  AND NOT EXISTS (
      SELECT 1
      FROM city_stage s
...
def load_city_stats(args: argparse.Namespace) -> dict[str, int]:
    ...
    if len(rows) != 1:
        raise RuntimeError(f"Expected one IE city stats row, got {len(rows)}")
    row = rows[0]
    return {
...
def main() -> int:
    args = parse_args()
    settlements = load_ie_settlements()
    if not settlements:
        print("No current IE settlements found in Wikidata.")
        return 0

    print(f"Loaded {len(settlements)} IE city/town settlements from Wikidata.")
    ...
    print(
        "Synced "
        f"{stats['city_count']} IE city rows "
